...
namespace Boologic.Levels
{
    ...
    public class Level_2 : MG_abstract
    {
        ...
        {   
            ...
            shape[7] = Content.Load<Texture2D>("table_or");
            shape_box[7] = new Rectangle (750, 220, shape[7].Width, shape[7].Height);
            shape[8] = Content.Load<Texture2D>("or_gate");
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if(mouse_state.LeftButton == ButtonState.Pressed && mouse_box.Intersects(button_box[0]) && old_mouse_state.LeftButton == ButtonState.Released)
                Settings_file.CurrentState = Settings_file.Layer.Level_3; //NEXT
        ...
        {    
            ...
            spriteBatch.DrawString(gameFont,"OR Gate Level",new Vector2(170,120),Color.LightGray);
            spriteBatch.DrawString(gameFont_2,"The OR gate is a digital logic gate that implements logical disjunction (v)",new Vector2(10,220),Color.LightGray);
            spriteBatch.DrawString(gameFont_2,"from mathematical logic. The behavior of operation is shown on the right,",new Vector2(10,255),Color.LightGray);
            spriteBatch.DrawString(gameFont_2,"in truth table. A HIGH output results if one or both the inputs to the gate",new Vector2(10,290),Color.LightGray);
            spriteBatch.DrawString(gameFont_2,"are HIGH. If neither input is high, a LOW output results. In another sense,",new Vector2(10,325),Color.LightGray);
            spriteBatch.DrawString(gameFont_2,"the function of OR effectively finds the maximum between two binary digits.",new Vector2(10,360),Color.LightGray);
            ...
            if(mouse_state.LeftButton == ButtonState.Pressed && mouse_box.Intersects(button_box[2]))
            {
                ...
                spriteBatch.DrawString(gameFont, "OR Order",new Vector2(340,700),Color.Black);
            }    
        }
    }
}