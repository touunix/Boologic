...
namespace Boologic.Levels
{
    public class Level_8 : MG_abstract
    {
        ...
        {   
            ...
            shape[3] = Content.Load<Texture2D>("shape8"); //NUMER POZIOMU
            ...
            shape[7] = Content.Load<Texture2D>("table_nand");
            shape_box[7] = new Rectangle (750, 220, shape[7].Width, shape[7].Height);
            shape[8] = Content.Load<Texture2D>("nand_gate");
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if(mouse_state.LeftButton == ButtonState.Pressed && mouse_box.Intersects(button_box[0]) && old_mouse_state.LeftButton == ButtonState.Released)
                Settings_file.CurrentState = Settings_file.Layer.Level_9; //NEXT
        ...
        {    
            ...
            spriteBatch.DrawString(gameFont,"NAND Gate Level",new Vector2(170,120),Color.LightGray);
            spriteBatch.DrawString(gameFont_2,"The NAND gate is a digital logic gate which produces an output which is false",new Vector2(10,220),Color.LightGray);
            spriteBatch.DrawString(gameFont_2,"only if all its inputs are true. The NAND gate is significant because any",new Vector2(10,255),Color.LightGray);
            spriteBatch.DrawString(gameFont_2,"boolean function can be implemented by using a combination of NAND gates.",new Vector2(10,290),Color.LightGray);
            spriteBatch.DrawString(gameFont_2,"The behavior of operation is shown on the right, in truth table.",new Vector2(10,325),Color.LightGray);
            spriteBatch.DrawString(gameFont_2,"Symbol of NAND gate:",new Vector2(150,420),Color.LightGray);
            ...
            if(mouse_state.LeftButton == ButtonState.Pressed && mouse_box.Intersects(button_box[2]))
            {
                ...
                spriteBatch.DrawString(gameFont, "NAND Order",new Vector2(260,700),Color.Black);
            }    
        }
    }
}